...
export default async function StaffMeetingDetailPage({
  params,
}: {
  ...
  const { data: rawMember, error: memberErr } = await supabase
    ...
    .eq('user_id', user.id)
    .maybeSingle();
  if (memberErr) console.error('[staff/solicitacao] member lookup failed', memberErr);
  ...
  const [reqRes, histRes, commentsRes] = await Promise.all([
    supabase.from('meeting_requests').select('*').eq('id', id).maybeSingle(),
    supabase.from('meeting_request_events').select('*').eq('meeting_request_id', id).order('created_at', { ascending: true }),
    supabase.from('meeting_request_comments').select('*').eq('meeting_request_id', id).order('created_at', { ascending: true }),
  ]);
  if (reqRes.error) console.error('[staff/solicitacao] request query failed', reqRes.error);
  if (histRes.error) console.error('[staff/solicitacao] history query failed', histRes.error);
  if (commentsRes.error) console.error('[staff/solicitacao] comments query failed', commentsRes.error);

  const request = reqRes.data as RequestRow | null;
  if (!request) notFound();

  // Hidrata membros envolvidos
  ...
  const { data: rawMembers, error: membersErr } = memberIds.length > 0
    ? await supabase.from('members').select('id, name, slug, color_hex, avatar_url, role').in('id', memberIds)
    : { data: [] as MemberFields[], error: null };
  if (membersErr) console.error('[staff/solicitacao] members query failed', membersErr);
  ...
  return (
    <MeetingDetailClient
      ...
      hasLoadError={!!(histRes.error || commentsRes.error || membersErr)}
    />
  );
}
